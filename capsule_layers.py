...
class DeconvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        if self.upsamp_type == 'resize':
            ...
            print(self.padding)
            out_height = deconv_output_length(self.input_height, stride=self.scaling, filter_size=self.kernel_size, padding=self.padding)
            out_width = deconv_output_length(self.input_height, stride=self.scaling, filter_size=self.kernel_size, padding=self.padding)
    ...
    def compute_output_shape(self, input_shape):
        ...
        output_shape[1] = deconv_output_length(output_shape[1], self.scaling, self.kernel_size, self.padding)
        output_shape[2] = deconv_output_length(output_shape[2], self.scaling, self.kernel_size, self.padding)
